...
For this unit you'll use **GroqCloud**. Groq runs popular open models like Llama on extremely fast hardware, and they have a generous free tier that doesn't ask for a credit card.
...
The free tier is rate-limited (you can only send so many requests per minute), but it is more than enough for this unit. They should not ask you for a credit card during signup. If something is asking you to pay, you might be on the wrong page.
...
- For this unit you'll paste it into the Colab notebook directly, which is fine because your Colab is private. For real apps you'll move keys into environment variables.
...
- **OpenAI, Anthropic, Google, or any provider you already have access to.** You will need to swap out the `groq` Python client for theirs and adjust the model name.
...
> **With your partner:** Have either of you used an AI API before? If yes, what did you build? If no, what project would you want to try first?
