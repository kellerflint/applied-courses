"""
V3: full V2 pipeline + bounding-box normalization applied to all images before splitting.
The model therefore trains and tests on the same normalized distribution.
Run with: python3.11 digit_test_v3.py
"""
...
# ─── 1. Load + bbox-normalize every image ─────────────────────────────────────
def bbox_normalize(arr):
    """Crop to stroke bounding box, pad slightly, resize back to 28x28."""
    g = arr[:, :, 0]
    rows = np.any(g > 0.2, axis=1)
    cols = np.any(g > 0.2, axis=0)
    if not rows.any():
        return arr
    r0, r1 = np.where(rows)[0][[0, -1]]
    c0, c1 = np.where(cols)[0][[0, -1]]
    r0 = max(0, r0 - BBOX_PAD)
    r1 = min(IMG_SIZE - 1, r1 + BBOX_PAD)
    c0 = max(0, c0 - BBOX_PAD)
    c1 = min(IMG_SIZE - 1, c1 + BBOX_PAD)
    cropped = g[r0:r1+1, c0:c1+1]
    pil = Image.fromarray((cropped * 255).astype(np.uint8)).resize(
...
for label_name in sorted(os.listdir(DATASET_PATH)):
    ...
    for fname in os.listdir(label_dir):
        ...
        images.append(bbox_normalize(arr))
...
plt.suptitle("V3 augmentation preview (bbox-normalized inputs)", fontsize=11, y=1.01)
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), "augmentation_preview_v3.png"), dpi=120, bbox_inches="tight")
plt.close()
print(f"Saved augmentation preview")
...
# Oversample 5s: repeat them 2x in training so the model gets more gradient on the hard class
fives_mask = y_tr == 5
X_tr = np.concatenate([X_tr, X_tr[fives_mask], X_tr[fives_mask]])
y_tr = np.concatenate([y_tr, y_tr[fives_mask], y_tr[fives_mask]])

print(f"\nTrain: {len(X_tr)} (with 5s oversampled 3x), Val: {len(X_val)}, Test: {len(X_test)}")
...
# ─── 8b. Test-time augmentation (TTA) ────────────────────────────────────────
# Average softmax over N augmented passes per image, then take argmax.
# Round 0 is the clean (unaugmented) prediction so it always has a vote.
TTA_ROUNDS = 25
print(f"\nRunning TTA ({TTA_ROUNDS} rounds)...")

softmax_accum = model.predict(X_test, verbose=0)  # round 0: clean
for r in range(TTA_ROUNDS - 1):
    aug_batch = np.array([augment(img) for img in X_test])
    softmax_accum += model.predict(aug_batch, verbose=0)
    if (r + 2) % 5 == 0:
        print(f"  round {r+2}/{TTA_ROUNDS}")
...
ax.set_title(f"V3 confusion matrix  |  TTA acc {tta_acc:.1%}  (baseline {test_acc:.1%})")
for i in range(10):
    for j in range(10):
        ax.text(j, i, str(cm[i, j]), ha="center", va="center",
                color="white" if cm[i, j] > cm.max() * 0.5 else "black", fontsize=9)
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), "confusion_matrix_v3.png"), dpi=120, bbox_inches="tight")
plt.close()
print(f"Saved confusion matrix")

# ─── 9b. Show misclassified images ───────────────────────────────────────────
...
if len(wrong_idx) > 0:
    ...
    confidences = softmax_accum / TTA_ROUNDS  # average softmax = confidence
    ...
    plt.suptitle(f"Misclassified images ({len(wrong_idx)} total)", fontsize=12, y=1.02)
    plt.tight_layout()
    err_path = os.path.join(os.path.dirname(__file__), "misclassified_v3.png")
    plt.savefig(err_path, dpi=140, bbox_inches="tight")
    plt.close()
...
plt.tight_layout()
plt.savefig(os.path.join(os.path.dirname(__file__), "training_curve_v3.png"), dpi=100, bbox_inches="tight")
plt.close()

# ─── 11. Export ───────────────────────────────────────────────────────────────
model_path = os.path.join(os.path.dirname(__file__), "digit_model_v3.keras")
